...
- A <body> containing:
  - A <header> with the main project title and a "Back to Home" button styled as a modern button that links to `/`.
  - A <nav> bar with anchor links to the different sections of the page.
  - A <main> section with the content from the Markdown, divided into logical <section> tags, each with a unique ID for the nav links.
  - A <footer> with the text 'Generated by Epirus Showcase Agent'.
